...
function jsPDFInvoiceTemplate(props) {
  ...
  doc.text(param.business.name, docWidth / 2, currentHeight, { align: "center" });
  //End Business Name
  //Business Info
  doc.setFontSize(pdfConfig.fieldTextSize);
  currentHeight += pdfConfig.lineHeight;
  doc.text(param.business.address, docWidth / 2, currentHeight, { align: "center" });
  currentHeight += pdfConfig.lineHeight;
  doc.text(param.business.phone, docWidth / 2, currentHeight, { align: "center" });
  currentHeight += pdfConfig.lineHeight;
  doc.text(param.business.email, docWidth / 2, currentHeight, { align: "center" });
  doc.setFontSize(pdfConfig.fieldTextSize);
  currentHeight += pdfConfig.lineHeight;
  doc.text(param.business.website, docWidth / 2, currentHeight, { align: "center" });
  //End Business Info
  //line breaker after business info
  if (param.invoice.header.length) {
    currentHeight += pdfConfig.subLineHeight;
    doc.line(10, currentHeight, docWidth - 10, currentHeight);
  }
  //End line breaker after business info
  doc.setTextColor(colorBlack);
  doc.setFontSize(pdfConfig.headerTextSize - 5);
  doc.text("Sales Report", docWidth / 2, currentHeight += pdfConfig.lineHeight, { align: "center" });
  //Contact part
  doc.setTextColor(colorGray);
  doc.setFontSize(pdfConfig.fieldTextSize);
  currentHeight += pdfConfig.lineHeight;
  let heightInvoce = currentHeight;
  doc.text("Customer Name: "+param.contact.name,10, currentHeight,);
  currentHeight += pdfConfig.lineHeight;
  doc.text("Customer Phone: "+param.contact.phone ,10, currentHeight,);
  currentHeight += pdfConfig.lineHeight;
  doc.text("Customer Id: "+param.contact.id ,10, currentHeight,);
  currentHeight += pdfConfig.lineHeight;
  doc.text("Customer Address: "+param.contact.address ,10, currentHeight,);
  // currentHeight += pdfConfig.lineHeight;

  /** Invoice */
  doc.text("Invoice: "+param.invoice.num ,docWidth - 10, heightInvoce,"right");
  heightInvoce += pdfConfig.subLineHeight;
  doc.text("Date: "+param.invoice.invDate ,docWidth - 10, heightInvoce,"right");
  /** End Invoice */
  /** Table */
  ...
  if (param.outputType === "save") doc.save(param.fileName);
    ...
    returnObj = {
      ...returnObj,
      dataUriString: doc.output("dataurlnewwindow", {
